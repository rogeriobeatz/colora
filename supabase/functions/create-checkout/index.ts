...
serve(async (req) => {
  ...
  try {
    ...
    // Buscar ou criar cliente Stripe pelo e-mail
    ...
    // Criar sessão de checkout com metadados para criação posterior do usuário
    const session = await stripe.checkout.sessions.create({
      customer: customerId,
      line_items: [
        {
          price: isSubscription
            ? "price_1T458zRjNIKJreFo2hsTiIKO" // Substitua pelo seu ID de preço real do Stripe
            : "price_1T459DRjNIKJreFoNCmabUQM",
          quantity: 1,
        },
      ],
      mode: isSubscription ? "subscription" : "payment",
      // Redirecionar para a página de callback de autenticação
      success_url: `${req.headers.get("origin")}/auth/callback?payment=success&email=${encodeURIComponent(customerData.email)}&session_id={CHECKOUT_SESSION_ID}`,
    ...
    logStep("Guest checkout session created", { sessionId: session.id });
